...
function mergeKeys(state, frame, source, sourceTag) {
  ...
  for (const sourceKey of sourceTag.keys(source)) {
    ...
    frame.overridable ?? (frame.overridable = /* @__PURE__ */ new Set());
...
function writeNode(state, level, node, parent, ctx) {
  ...
  if (node.kind === "mapping") if (useBlockCollection) body = writeBlockMapping(state, level, node, compact);
    ...
    shouldPrintTag = node.tagged || body === "" && layout.flowOnly && (parent == null ? void 0 : parent.kind) === "sequence" && !hasAnchor || layout.style !== SCALAR_STYLE.PLAIN && node.tag !== state.defaultScalarTagName;
  }
